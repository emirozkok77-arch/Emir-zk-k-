...
def init_files():
    ...
    # Tüm dosyaları güvenli modda başlat
...
<style>
    ...
    /* GİRİŞ VE KAYIT KUTULARI (YAN YANA) */
    .auth-box {
...
# ==========================================
# 1. LANDING PAGE (YENİ DÜZEN: SOL GİRİŞ - SAĞ KAYIT)
# ==========================================
if st.session_state.page == 'landing' and not st.session_state.logged_in:
    
    # FOTOĞRAF VE BAŞLIK (ORTADA)
    ...
    with c_img:
        ...
        if photo_path:
            ...
            st.markdown(f'''<div style="width:100%; aspect-ratio: 1/1; overflow:hidden; border-radius:50%; border:3px solid #3b82f6; box-shadow: 0 0 20px rgba(59, 130, 246, 0.5);"><img src="data:image/png;base64,{encoded_string}" style="width:100%; height:100%; object-fit:cover;"></div>''', unsafe_allow_html=True)
    
    with c_tit:
        st.markdown("<h1 style='font-size: 60px; color:#3b82f6; margin:0; padding-top:20px;'>EMİR ÖZKÖK</h1>", unsafe_allow_html=True)
    
    st.markdown("""
    <div style='text-align:center; margin: 30px 0; padding: 0 5%;'>
    ...
    # --- AYRIK GİRİŞ VE KAYIT KUTULARI ---
    c_login, c_register = st.columns(2, gap="large")
    ...
    with c_register:
        ...
        rt = c_r1.text_input("Telefon", key="r_t", max_chars=11)
        rm = c_r2.text_input("E-posta", key="r_m")
